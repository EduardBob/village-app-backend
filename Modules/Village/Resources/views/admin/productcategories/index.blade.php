...
@section('content-header')
    <h1>
        {{ trans('village::productcategories.title.productcategories') }}
    </h1>
    <ol class="breadcrumb">
        <li><a href="{{ route('dashboard.index') }}"><i class="fa fa-dashboard"></i> {{ trans('core::core.breadcrumb.home') }}</a></li>
        <li class="active">{{ trans('village::productcategories.title.productcategories') }}</li>
    </ol>
@stop
        ...
        <div class="col-xs-12">
            <div class="row">
                <div class="btn-group pull-right" style="margin: 0 15px 15px 0;">
                    <a href="{{ route('admin.village.productcategory.create') }}" class="btn btn-primary btn-flat" style="padding: 4px 10px;">
                        <i class="fa fa-pencil"></i> {{ trans('village::productcategories.button.create productcategory') }}
                    </a>
                </div>
            </div>
...
@section('shortcuts')
    <dl class="dl-horizontal">
        <dt><code>c</code></dt>
        <dd>{{ trans('village::productcategories.title.create productcategory') }}</dd>
    </dl>
@stop
    ...
    <script type="text/javascript">
        $( document ).ready(function() {
            $(document).keypressAction({
                actions: [
                    { key: 'c', route: "<?= route('admin.village.productcategory.create') ?>" }
                ]
            });
        });
    </script>

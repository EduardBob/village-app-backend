...
class SidebarExtender implements \Maatwebsite\Sidebar\SidebarExtender
{
    ...
    public function extendWith(Menu $menu)
    {
        $menu->group(trans('core::sidebar.content'), function (Group $group) {
            $group->item(trans('village::village.title'), function (Item $item) {
                $item->icon('fa fa-home');
                $item->weight(10);
                $item->authorize(
                     /* append */
                );
                $item->item(trans('village::users.title.users'), function (Item $item) {
                    $item->icon('fa fa-copy');
                    $item->weight(0);
                    $item->append('admin.village.user.create');
                    $item->route('admin.village.user.index');
                    $item->authorize(
                        $this->auth->hasAccess('village.users.index')
                    );
                });
